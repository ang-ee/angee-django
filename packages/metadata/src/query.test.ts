...
describe("ResourceQuery", () => {
  ...
  test("separates identity, label, selected row paths and server bucket names", () => {
    ...
    const rows = [
      { channel: { id: "chn_a", display_name: "Team" } },
      { channel: { id: "chn_b", display_name: "Team" } },
      { channel: null },
    ];
    expect(rows.map((row) => axis.identity(row))).toEqual([
      "chn_a",
      "chn_b",
      null,
    ]);
    expect(rows.map((row) => axis.label(row))).toEqual(["Team", "Team", null]);
    expect(query.selection([{ field: "channel" }])).toEqual([
      "channel.id",
      "channel.display_name",
    ]);
    expect(axis.groupBy()).toEqual({
      dimensions: [
        { input: "CHANNEL", key: "channel_id" },
        { input: "CHANNEL__DISPLAY_NAME", key: "channel__display_name" },
      ],
      valueKey: "channel_id",
      labelKey: "channel__display_name",
      orderBy: [
        { field: "channel__display_name", direction: "ASC", nulls: "LAST" },
        { field: "channel_id", direction: "ASC", nulls: "LAST" },
      ],
    });
    expect(query.toWhere(axis.drill({ key: { channel_id: "chn_a" } }))).toEqual(
      { channel: { _eq: "chn_a" } },
    );
    expect(axis.drill({ key: { channel_id: null } })).toEqual({
      channel: { isNull: true },
    });
    expect(() => axis.identity({})).toThrow(/missing selected identity path/);
    expect(() => axis.identity({ channel: { id: {} } })).toThrow(
      /must be a scalar/,
    );
    expect(() => axis.label({ channel: { id: "chn_a" } })).toThrow(
      /missing selected label path/,
    );
  });
  test("distinguishes scalar relations, client-only and summary-only axes", () => {
    const query = ResourceQuery.from(
      testDataResource("storage.File", {
    ...
    expect(() => query.axis("drive").groupBy()).toThrow(
      /does not support server/,
    );
    expect(
      query.axis("owner").drill({ key: { channel_id: "chn_a" } }),
    ).toBeUndefined();
    expect(() => query.axis("owner").selection).toThrow(
  ...
  test("rejects legacy aliases, unknown fields and unsupported operators", () => {
    ...
    for (const filter of [
      ...
      expect(() => query.filterFrom(filter)).toThrow(QueryParseError);
    expect(() =>
      query.groupsFrom([{ field: "channel", aggregateKey: "channel_id" }]),
    ).toThrow(QueryParseError);
    expect(() =>
      query.groupsFrom([{ field: "channel" }, { field: "channel" }]),
    ).toThrow(/duplicate/);
    expect(
      query
        .groupsFrom([{ field: "channel.display_name" }])
        .map((axis) => axis.id),
    ).toEqual(["channel"]);
    expect(() => query.groupsFrom([{ field: "channel.nope" }])).toThrow(
      /unknown group/,
    );
    expect(() =>
      query.groupsFrom([{ field: "channel__display_name" }]),
    ).toThrow(/unknown group/);
  });
  test("preserves empty membership, null predicates and boolean branches", () => {
    const query = ResourceQuery.from(resource());
    expect(query.toWhere()).toEqual({});
    expect(query.toWhere({ body: { inList: [] } })).toEqual({
      body: { _in: [] },
    });
    expect(query.toWhere({ OR: [] })).toEqual({ _or: [] });
    expect(query.toWhere({ AND: [] })).toEqual({ _and: [] });
    expect(query.toWhere({ NOT: { channel: null } })).toEqual({
      _not: { channel: { _is_null: true } },
    });
    expect(() => query.toWhere({ body: { exact: null } })).toThrow(
      /use isNull/,
    );
    expect(() => query.toWhere({ body: { inList: [null] } })).toThrow(
      /use isNull/,
    );
    expect(() => query.toWhere({ body: { isNull: "true" } })).toThrow(
      /expected a boolean/,
    );
  ...
  test("uses filter input types and enum mappings independently of output symbols", () => {
    const query = ResourceQuery.from(
      testDataResource("notes.Note", {
        query: testResourceQuery({
          fields: {
            state: field("state", "String", ["exact"], {
              kind: "enum",
              values: [{ value: "IN_REVIEW" }],
              filter: {
                field: "state",
                scalar: "String",
                values: [],
    ...
    expect(query.toWhere({ state: { exact: "in_review" } })).toEqual({
      state: { _eq: "in_review" },
    });
    expect(
  ...
  test("requires selected values and distinguishes absent JSON keys from omitted selections", () => {
    ...
    expect(() => query.matches({}, { body: { isNull: true } })).toThrow(
      /missing selected value path/,
    );
    expect(query.matches({ body: null }, { body: { isNull: true } })).toBe(
      true,
    );
    const jsonQuery = ResourceQuery.from(
      testDataResource("notes.Note", {
        query: testResourceQuery({
          fields: {
            "metadata.mailbox": field(
              "metadata.mailbox",
              "String",
              ["exact", "isNull"],
              { row: { path: "metadata.mailbox", paths: ["metadata"] } },
            ),
          },
          axes: {
            "metadata.mailbox": {
    ...
    expect(
      jsonQuery.matches(
        { metadata: {} },
        { "metadata.mailbox": { isNull: true } },
      ),
    ).toBe(true);
    expect(
      jsonQuery.axis("metadata.mailbox").identity({ metadata: {} }),
    ).toBeNull();
    expect(() => jsonQuery.axis("metadata.mailbox").identity({})).toThrow(
      /missing selected identity path/,
    );
  });
  test("sorts only via declared canonical paths and exact wire keys", () => {
    ...
    expect(query.toSorters([{ field: "count", direction: "DESC" }])).toEqual([
      { field: "message_count", order: "desc" },
    ]);
    expect(() => query.toOrderBy({ message_count: "asc" })).toThrow(
      /cannot be sorted/,
    );
  ...
  test("removes selected facet constraints while preserving other boolean branches", () => {
    const query = ResourceQuery.from(resource());
    expect(
      query.toFacet("channel", {
        channel: { exact: "a" },
        body: { contains: "urgent" },
      }).where,
    ).toEqual({ body: { _like: "%urgent%" } });
    expect(
      query.withoutFields({ OR: [{ channel: "a" }, { body: "urgent" }] }, [
        "channel",
      ]),
    ).toEqual({ OR: [{}, { body: { exact: "urgent" } }] });
...
describe("date and JSON axes", () => {
  test("shares UTC identity with half-open aggregate-owned range predicates", () => {
    const metadata = resource();
    const drill: QueryDrill = {
      kind: "range",
      field: "sent_at",
      valueKey: "sent_at_month",
      rangeKey: "sent_at_month_range",
      nullMode: "isNull",
      valueMap: [],
    };
    metadata.query.axes.sent_at = {
      field: "sent_at",
      kind: "date",
      identityPath: "sent_at",
      paths: ["sent_at"],
      server: { input: "SENT_AT", key: "sent_at" },
      extractions: [
        {
          name: "month",
          input: "MONTH",
    ...
    const bucket = {
      key: {
        sent_at_month: "2026-02-01 00:00:00",
        sent_at_month_range: { from: "2026-02-01", to: "2026-03-01" },
      },
    };
    expect(axis.identity({ sent_at: "2026-03-01T00:30:00+02:00" })).toBe(
      "2026-02",
    );
    expect(axis.bucketIdentity(bucket)).toBe("2026-02");
    expect(query.toWhere(axis.drill(bucket))).toEqual({
      sent_at: {
        _gte: "2026-02-01T00:00:00.000Z",
        _lt: "2026-03-01T00:00:00.000Z",
      },
    });
    expect(() => axis.drill({ key: { sent_at_month: "2026-02-01" } })).toThrow(
      /missing range/,
    );
    expect(axis.drill({ key: { sent_at_month: null } })).toEqual({
      sent_at: { isNull: true },
    });
  });
  test("separates JSON row paths from legal selections and SQL null from JSON null", () => {
    const metadata = resource();
    metadata.query.axes["metadata.mailbox"] = {
      field: "metadata.mailbox",
      kind: "json",
      identityPath: "metadata.mailbox",
      paths: ["metadata"],
      server: { input: "METADATA__MAILBOX", key: "metadata__mailbox" },
      extractions: [],
      drill: {
        kind: "json",
        field: "metadata",
    ...
    expect(
      query.toWhere(axis.drill({ key: { metadata__mailbox: null } })),
    ).toEqual({ metadata: { _contains: { mailbox: null } } });
  });
});

describe("local queries and filter values", () => {
  const query = ResourceQuery.forRows({
    fields: {
      body: { scalar: "String" },
      count: { scalar: "Int" },
      metadata: { kind: "json" },
      ...
      owner: {
        kind: "relation",
        identityPath: "owner.key",
        labelPath: "owner.name",
      },
    },
  });
  test("evaluates scalar, relation, wildcard and JSON filters at the same owner", () => {
    const row = {
      body: "Alpha%beta",
      count: 3,
      metadata: { mailbox: null, tags: ["urgent", "team"] },
      owner: { key: "usr_a", name: "A" },
    };
    expect(
      query.matches(row, { body: { iContains: "ALPHA%" }, count: { gte: 3 } }),
    ).toBe(true);
    expect(query.matches(row, { owner: { exact: "usr_a" } })).toBe(true);
    expect(
      query.matches(row, {
        metadata: { jsonContains: { mailbox: null, tags: ["team"] } },
      }),
    ).toBe(true);
    expect(
      query.matches(row, { metadata: { jsonContains: { tags: ["missing"] } } }),
    ).toBe(false);
  ...
  test("does not interpret free text as dates or guess relation identities", () => {
    expect(query.axis("body").identity({ body: "2026-09-07" })).toBe(
      "2026-09-07",
    );
    expect(
      query
        .axis("sent_at", "week")
        .identity({ sent_at: "2026-09-06T20:00:00Z" }),
    ).toBe("2026-08-31");
    expect(() =>
      query.axis("owner").identity({ owner: { id: "wrong-field" } }),
    ).toThrow(/owner.key/);
  });
  test("copies caller state and preserves conflicting constraints", () => {
    ...
    expect(query.toWhere(conflict)).toEqual({
      _and: [{ body: { _eq: "a" } }, { body: { _eq: "b" } }],
    });
  });
  test("toggles facets with one identity comparison shape", () => {
    ...
    expect(
      Filter.from(first).toggleFacet({ ...facet, value: "chn_b" }),
    ).toEqual({ channel: { inList: ["chn_a", "chn_b"] } });
...
test("editing or clearing text preserves sibling comparisons", () => {
  ...
  expect(Filter.from(edited).withTextTerm("  ", "title")).toEqual({
    title: { startsWith: "A", isNull: false },
    active: { exact: true },
  });
  expect(
    Filter.from({ name: { iContains: "text" } }).withTextTerm("", "name"),
  ).toEqual({});
...
test("datetime equality and groups compare instants without losing microseconds", () => {
  const query = ResourceQuery.forRows({
    fields: { time: { scalar: "DateTime" } },
  });
  const row = { time: "2026-09-07T10:00:00Z" };
  expect(
    query.matches(row, { time: { exact: "2026-09-07T12:00:00+02:00" } }),
  ).toBe(true);
  expect(
    query.matches(row, { time: { inList: ["2026-09-07T12:00:00+02:00"] } }),
  ).toBe(true);
  expect(
    query.matches(row, { time: { exact: "2026-09-07T10:00:00.000001Z" } }),
  ).toBe(false);
  expect(query.axis("time").identity(row)).toBe(
    query.axis("time").identity({ time: "2026-09-07T12:00:00+02:00" }),
  );
});

test("date operands follow native calendar types without JavaScript rollover", () => {
  const query = ResourceQuery.forRows({
    fields: { date: { scalar: "Date" }, time: { scalar: "DateTime" } },
  });
  for (const value of [
    "2026-02-30",
    "2026-02-29",
    ...
    expect(() => query.filterFrom({ time: `${value}T00:00:00Z` })).toThrow(
      /ISO date/,
    );
  }
  expect(() => query.filterFrom({ date: "2026-09-07T12:00:00Z" })).toThrow(
    /ISO date/,
  );
  expect(() => query.filterFrom({ time: "2026-09-07\n" })).toThrow(/ISO date/);
  expect(() =>
    query.filterFrom({ time: { inList: ["2026-09-07T12:34:56Z\n"] } }),
  ).toThrow(/ISO date/);
  expect(query.filterFrom({ date: "2024-02-29" })).toEqual({
    date: { exact: "2024-02-29" },
  });
  expect(
    query.matches({ time: "2026-09-07T00:00:00Z" }, { time: "2026-09-07" }),
  ).toBe(true);
  expect(
    query.matches(
      { time: "2026-09-08T00:00:00Z" },
      { time: "2026-09-07T24:00:00Z" },
    ),
  ).toBe(true);
  expect(
    query.matches(
      { time: "2026-09-07T00:00:00.123456Z" },
      { time: "2026-09-07T00:00:00.123456789Z" },
    ),
  ).toBe(true);
  expect(() =>
    query.axis("time", "month").identity({ time: "2026-02-30" }),
  ).toThrow(/invalid date/);
});

test("numeric filters reject values outside native GraphQL input domains", () => {
  const query = ResourceQuery.forRows({
    fields: { count: { scalar: "Int" }, ratio: { scalar: "Float" } },
  });
  for (const value of [-(2 ** 31), 0, 2 ** 31 - 1]) {
    expect(query.toWhere({ count: { exact: value, inList: [value] } })).toEqual(
      { count: { _eq: value, _in: [value] } },
    );
  }
  ...
  expect(query.toWhere({ ratio: { exact: Number.MAX_VALUE } })).toEqual({
    ratio: { _eq: Number.MAX_VALUE },
  });
  for (const value of [NaN, Infinity, -Infinity]) {
    expect(() => query.toWhere({ ratio: { exact: value } })).toThrow(
      QueryParseError,
    );
  }
});
...
test("time filters validate native ISO clocks without losing offsets or precision", () => {
  ...
  for (const value of [
    ...
    "23:59:59.123456789",
    "12:34:56Z",
    "12:34:56+05:30:15.123456",
  ]) {
    expect(query.toWhere({ time: { exact: value, inList: [value] } })).toEqual({
      time: { _eq: value, _in: [value] },
    });
  }
  for (const value of [
    "not-a-time",
    ...
    expect(() => query.toWhere({ time: { inList: [value] } })).toThrow(
      /ISO time/,
    );
  }
});
