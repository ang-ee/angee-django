...
function metadata(
  ...
  rowModel: "client" | "server" = "server",
  recordSearchFields: readonly string[] = [],
) {
...
describe("resolveTextFilterField", () => {
  ...
  it("uses the first resource-authored search field as the semantic text term", () => {
    const supplierReference = {
      ...text,
      filter: { ...text.filter!, field: "supplier_reference" },
    };
    expect(resolveTextFilterField(metadata(
      { supplier_reference: supplierReference },
      "server",
      ["supplier_reference"],
    ))).toBe("supplier_reference");
  });
