...
contract.axes.party!.server = {
  input: "PARTY",
  key: "partyId",
  labelInput: "PARTY__DISPLAY_NAME",
  labelKey: "party_DisplayName",
};
contract.axes.party!.drill = {
  kind: "identity",
  field: "party",
  valueKey: "partyId",
  nullMode: "isNull",
...
contract.axes.createdAt!.extractions = contract.axes.createdAt!.extractions.map(
  (extraction) => ({
    ...extraction,
    key: `createdAt${extraction.name}`,
    rangeKey: `createdAt${extraction.name}Range`,
    drill: {
      kind: "range",
      field: "createdAt",
      valueKey: `createdAt${extraction.name}`,
      rangeKey: `createdAt${extraction.name}Range`,
      nullMode: "isNull",
      valueMap: [],
    },
...
contract.fields.metadata!.filter = {
  ...contract.fields.metadata!.filter!,
  operators: ["exact", "jsonContains", "isNull"],
};
contract.axes["metadata.mailbox"]!.server = {
  input: "METADATA__MAILBOX",
  key: "metadata__mailbox",
};
contract.axes["metadata.mailbox"] = {
  ...contract.axes["metadata.mailbox"]!,
  paths: ["metadata"],
};
contract.axes["metadata.mailbox"]!.drill = {
  kind: "json",
  field: "metadata",
...
contract.axes.nestedOwner!.server = {
  input: "NESTED_OWNER",
  key: "nestedOwner",
};
...
describe("resource query grouping projections", () => {
  test("uses declared wire inputs and keys without spelling inference", () => {
    expect(query.axis("status").groupBy()).toEqual({
      dimensions: [{ input: "SERVER_STATUS", key: "serverStatus" }],
      valueKey: "serverStatus",
  ...
  test("accepts only an axis's identity path as an alias for the declared axis", () => {
    expect(() => query.groupsFrom([{ field: "party.display_name" }])).toThrow(/unknown group/);
    expect(query.groupsFrom([{ field: "party.id" }]).map((axis) => axis.spec)).toEqual([{ field: "party" }]);
    expect(query.canonicalAxisField("party.display_name")).toBeUndefined();
    expect(query.canonicalAxisField("party.nope")).toBeUndefined();
    expect(() => query.groupsFrom([{ field: "party" }, { field: "party.id" }])).toThrow(/duplicate/);
  });
  test("rejects unknown axes at the boundary", () => {
    expect(() =>
      query.groupsFrom([{ field: "party", aggregateKey: "partyId" }]),
    ).toThrow();
    expect(() => query.group({ field: "missing" })).toThrow(
      "unknown group axis",
    );
  });
  ...
  test("duplicate relation names preserve identities and independent predicates", () => {
    const first = {
      key: { partyId: "1", party_DisplayName: "Same" },
      count: 1,
    };
    const second = {
      key: { partyId: "2", party_DisplayName: "Same" },
      count: 1,
    };
    expect(
      bucketValueLabels(
    ...
    expect(query.toWhere(query.axis("party").drill(first))).toEqual({
      party: { _eq: "1" },
    });
    expect(query.toWhere(query.axis("party").drill(second))).toEqual({
      party: { _eq: "2" },
    });
  });
...
describe("localized labels over stable identities", () => {
  test.each([
    ...
    ["month", "2026-08"],
    ["week", "2026-08-17"],
  ])("%s is computed once by the axis", (granularity, identity) => {
    expect(
      query
        .axis("createdAt", granularity)
  ...
  test("client and server date values use the same localized renderer", () => {
    ...
    const alternateT = (key: string, vars?: Record<string, unknown>) =>
  ...
  test("declared local rows use the same semantic axis without server dimensions", () => {
    const [axis] = tableGroupAxes(
      [{ field: "createdAt", granularity: "month" }],
      null,
      [{ field: "createdAt" }],
    );
    expect(axis!.identity({ createdAt: "2026-09-01T00:00:00Z" })).toBe(
      "2026-09",
    );
