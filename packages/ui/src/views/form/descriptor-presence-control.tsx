...
export function DescriptorPresenceControl({ field, value, readOnly, onChange, children }: {
  ...
  if ((field.omittable || field.nullable) && value === undefined) {
    return <div className="flex items-center gap-2">
      ...
      {!readOnly ? <Button type="button" size="sm" variant="secondary"
        onClick={() => onChange(initialFormSpecValue({ ...field, nullable: field.hasDefault ? field.nullable : false } as FormSpecFieldDescriptor))}>
      ...
      {!readOnly && field.nullable ? <Button type="button" size="sm" variant="ghost"
        onClick={() => onChange(null)}>{t("form.value.leaveEmpty")}</Button> : null}
    </div>;
  }
    ...
    return <>
      ...
      {!readOnly ? <div className="mt-1 flex flex-wrap gap-1">
        <Button type="button" size="sm" variant="ghost"
          onClick={() => onChange(initialFormSpecValue({ ...field, nullable: false, hasDefault: false } as FormSpecFieldDescriptor))}>
          {t("form.value.set")}
        </Button>
        {field.omittable ? <Button type="button" size="sm" variant="ghost"
          onClick={() => onChange(undefined)}>{t("form.value.notSet")}</Button> : null}
      </div> : null}
    </>;
  }
