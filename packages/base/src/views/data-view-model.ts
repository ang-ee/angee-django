...
export class DataViewState {
  ...
  toSearch(initial: DataViewInitialState = {}): DataViewSearch {
    const search: DataViewSearch = {};
    const defaultPageSize = defaultDataViewPageSize(initial);
...
export function dataViewStateToSearch(
  state: DataViewState,
  initial: DataViewInitialState = {},
): DataViewSearch {
...
function defaultDataViewPageSize(initial: DataViewInitialState): number {
  return clampPageSize(initial.pageSize ?? DEFAULT_DATA_VIEW_PAGE_SIZE);
}
