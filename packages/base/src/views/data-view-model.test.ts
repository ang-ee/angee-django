...
describe("data-view model", () => {
  ...
  test("serializes relative to page-owned default view and page size", () => {
    ...
    const defaultState = DataViewState.create(defaults);
    expect(dataViewStateToSearch(defaultState, defaults)).toEqual({});

    const listState = defaultState.reduce({ type: "setView", view: "list" });
    expect(dataViewStateToSearch(listState, defaults)).toEqual({
      view: "list",
    });

    const resized = defaultState.reduce({ type: "setPageSize", pageSize: 50 });
    expect(dataViewStateToSearch(resized, defaults)).toEqual({
      pageSize: 50,
    });
  });
