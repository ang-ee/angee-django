...
- `docs/backend/guidelines.md` — the Django-Native Rule, package layering, naming,
...
The gold standard is how a senior Django core developer would write it using
Django's own owners. Look hardest for:
...
the repo's own checks (`ruff`, `mypy`, `pytest`) to ground a finding rather than
guess. Prefer a few high-confidence, specific findings over many vague ones. Be
skeptical and do not praise. If a recommendation might misread a Django idiom or a
...
- **Problem** — what's wrong and which doc rule or Django idiom it breaks
