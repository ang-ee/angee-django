...
- Superseded draft detail: the legacy VCS child was to become **`VcsBridge`** as a companion. Current MTI code instead makes `VcsBridge` the concrete integration child; repository inventory points to `Repository.vcs_bridge`.
...
- **GraphQL** (`integrate/schema.py`, `agents/schema.py`): `IntegrationType` drops `capability_statuses`, gains `impl_class` + the nested companion 1:1 (`bridge`/`inferenceProvider`); reshape the VCS bridge input/patch and `InferenceProviderInput/Patch` to companion shape; keep `changes(Integration, field="integrationChanged")` (1510); add the create path (B3) and `connectIntegration` (A4).
- **REBAC — superseded companion note.** Current MTI code keeps child resource
  types, but the VCS owner is now `integrate/vcs_bridge`, and
  `integrate/repository` field-derives through `relation vcs_bridge`.
