...
| S50 Move row date coercion to date-format owner | same commit | -7 prod, +3 tests; final total audit deferred | list, timeline, and form subtitle displays stop parsing date-ish row values locally | `widgets/date-format` owns valid Date/ISO/epoch coercion through date-fns; list/timeline/form owners keep grouping, bucket, and subtitle decisions | frontend + architecture pass; helper intentionally kept internal | base focused tests; base typecheck; `git diff --check` |
| S51 Share Integration inherited field write boundary | same commit | -78 prod, +15 tests; final total audit deferred | integration, VCS bridge, and inference-provider custom mutations stop repeating vendor/owner/credential/account/status public-id resolution | integrate GraphQL helpers own inherited Integration create/patch field coercion; child resolvers keep backend/config/default materialization | backend + architecture pass after provider-policy cleanup | focused agents/integrate GraphQL pytest; ruff; schema check; `git diff --check` |
