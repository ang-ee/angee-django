...
- ⏳ **`fragments/` (30 in P1) — IN PROGRESS, batched** (composites built FROM the primitives/page/layouts):
  ...
  - [ ] done [ ] verified [ ] checked by human (final qa) — **batch 2 (panels/composites)**: SurfacePanel, FocusPanel, RailPanel, ListPanel, HeroPanel, MarketingHero, LoadingPanel, EditorHeaderBar, FrameToolbar, DataLens, DialogForm, BrandButton + stories
  - [ ] done [ ] verified [ ] checked by human (final qa) — **batch 3 (specialized, defer to stack libs)**: JsonViewer (react-json-view-lite), LogPanel (ansi-to-react), GraphCanvas + GraphIntrospection (@xyflow/react) + stories
