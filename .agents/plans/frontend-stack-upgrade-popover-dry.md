...
> - **GAP A — column chooser ("Visible fields"):** the mockup control band (right group) has a `Visible fields` control; ours has **none** (grep of `ListView`/`DataToolbar` = no column-visibility/chooser). The user's named gap. → add a visible-fields chooser to `DataToolbar` (toggles `ListView` column visibility; last visible column can't be hidden — preservation gate).
> - **GAP B — view-switcher kinds:** ours = `List · Board` (DataViewKind, 2); mockup = `List · Kanban · Form · Calendar · Pivot · Graph` (6). → extend `DataViewKind` + build the missing view kinds (Graph needs the deferred `@xyflow/react`).
