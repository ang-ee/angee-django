...
- id: pkgbase-001
  ...
  resolution: The hardcoded STATUS_ORDER enum IS removed — base now keys off `column.tone` order, so the foreign note-status vocabulary no longer ships in the framework. What remains deferred (TODO in base): deciding WHICH field is a row's title/status for a generic list. That metadata is schema/addon-owned and the backend does not emit it yet; a frontend `status`/`title` heuristic would be a guess and would violate find-the-owner. Follow-up (real owner): the GraphQL schema should annotate title/status fields, then the list view reads the annotation. Tracked in RESOLUTION-LOG.md.
