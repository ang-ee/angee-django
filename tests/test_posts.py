"""Tests for the posts overlay on top of messaging's ingest.

Posts reuses one messaging write path (a public post *is* a ``messaging.Message``)
and adds only the overlay it owns. These cases pin that owned surface: the atomic
API-unit ledger (:class:`~angee.posts.managers.QuotaManager`), the open/close
following edge (:class:`~angee.posts.managers.FeedFollowManager`), the rolled-up
:class:`~angee.posts.models.PostMetrics` an ingest overlays, and
...
def _handle(value: str = "chan-1") -> Any:
    """Create one YouTube handle the posts following edge hangs off."""
...
def test_quota_consume_debits_the_budget(posts_tables: None) -> None:
    """consume() opens the period and debits quota_used by the spent units."""

    del posts_tables
    with system_context(reason="test quota consume"):
        integration = _feed("quota-debit")
        assert Quota.objects.consume(integration=integration, units=3000, limit=10000, now=_AT) is True
        ledger = Quota._base_manager.get(integration=integration)
...
@pytest.mark.django_db(transaction=True)
def test_quota_consume_refuses_when_exhausted_and_leaves_ledger_untouched(posts_tables: None) -> None:
    """A spend that would exceed the budget returns False without debiting."""

    del posts_tables
    with system_context(reason="test quota exhaustion"):
        integration = _feed("quota-exhaustion")
        assert Quota.objects.consume(integration=integration, units=6000, limit=10000, now=_AT) is True
        # 6000 + 5000 > 10000 → refused, ledger stays at the prior 6000.
        assert Quota.objects.consume(integration=integration, units=5000, limit=10000, now=_AT) is False
        after_refusal = Quota._base_manager.get(integration=integration).quota_used
        # 6000 + 4000 == 10000 → the budget is spendable up to the limit.
        assert Quota.objects.consume(integration=integration, units=4000, limit=10000, now=_AT) is True
        exhausted = Quota._base_manager.get(integration=integration).quota_used
...
@pytest.mark.django_db(transaction=True)
def test_quota_openers_converge_on_the_window_boundary(
    posts_tables: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Openers that both miss the initial read share one deterministic period and budget."""

    del posts_tables
    queryset_class = type(Quota.objects.all())
    monkeypatch.setattr(queryset_class, "current", lambda self, *, now: self.none())
    first_at = _AT
    second_at = _AT + timedelta(microseconds=1)
    with system_context(reason="test quota concurrent openers"):
        integration = _feed("quota-concurrent-openers")
        first = Quota.objects.open_period(integration=integration, limit=10000, now=first_at)
        second = Quota.objects.open_period(integration=integration, limit=10000, now=second_at)
        assert Quota.objects.consume(integration=integration, units=3000, limit=10000, now=first_at) is True
        assert Quota.objects.consume(integration=integration, units=4000, limit=10000, now=second_at) is True
        ledger = Quota._base_manager.get(integration=integration)

    assert first.pk == second.pk == ledger.pk
    assert ledger.period_start == datetime(2026, 1, 1, tzinfo=timezone.utc)
    assert ledger.period_end == datetime(2026, 1, 2, tzinfo=timezone.utc)
    assert ledger.quota_used == 7000
    assert Quota._base_manager.filter(integration=integration).count() == 1


@pytest.mark.django_db(transaction=True)
def test_quota_consume_uses_the_backend_gated_lock_contract(
    posts_tables: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Quota consumption locks through locked_get so unsupported backends remain explicit."""

    del posts_tables
    manager_class = type(Quota.objects)
    original = manager_class.locked_get
    locked_pks: list[Any] = []

    def locked_get(manager: Any, *args: Any, **kwargs: Any) -> Any:
        locked_pks.append(kwargs.get("pk"))
        return original(manager, *args, **kwargs)

    monkeypatch.setattr(manager_class, "locked_get", locked_get)
    with system_context(reason="test quota lock contract"):
        integration = _feed("quota-lock-contract")
        assert Quota.objects.consume(integration=integration, units=1, limit=10, now=_AT) is True
        ledger = Quota._base_manager.get(integration=integration)

    assert locked_pks == [ledger.pk]


@pytest.mark.django_db(transaction=True)
def test_quota_read_derives_from_its_integration(posts_tables: None) -> None:
    """An integration owner reads its quota even when the system-created row has no creator."""

    del posts_tables
    with system_context(reason="test quota integration access"):
        integration = _feed("quota-integration-access")
        quota = Quota.objects.open_period(integration=integration, limit=10, now=_AT)
    assert quota.created_by_id is None

    with actor_context(integration.owner):
        visible = Quota.objects.filter(pk=quota.pk).exists()
...
@pytest.mark.django_db(transaction=True)
def test_feed_follow_opens_the_edge_and_is_idempotent(posts_tables: None) -> None:
...
@pytest.mark.django_db(transaction=True)
def test_feed_unfollow_closes_the_interval_then_follow_reopens_it(posts_tables: None) -> None:
...
@pytest.mark.django_db(transaction=True)
def test_feed_sync_overlays_post_metrics_and_public_payload(posts_tables: None) -> None:
    ...
    StubFeedBackend.queue(
        feed,
        [_post("p1", subject_url="https://youtu.be/p1", metrics=metrics)],
    )
...
def test_thread_public_extension_has_no_json_tags_column() -> None:
    """Public-post hashtags no longer occupy a JSON column on messaging.Thread."""

    assert "tags" not in {field.name for field in ThreadPublic._meta.local_fields}


@pytest.mark.django_db(transaction=True)
def test_feed_sync_preserves_hashtags_in_message_metadata(posts_tables: None) -> None:
    """Feed ingest keeps exact hashtag order, spelling, and duplicates in the lossless envelope."""

    del posts_tables
    feed = _feed()
    tags = ("#Django", "angee", "#Django")
    StubFeedBackend.queue(feed, [_post("p1", tags=tags)])

    with system_context(reason="test feed sync hashtag envelope"):
        feed.sync()
        message = Message._base_manager.get(external_id="p1")

    assert message.metadata["tags"] == list(tags)


@pytest.mark.django_db(transaction=True)
def test_feed_sync_lands_public_threads_without_minting_quote_edges(posts_tables: None) -> None:
...
@pytest.mark.django_db(transaction=True)
def test_feed_sync_lands_public_visibility(posts_tables: None) -> None:
...
@pytest.mark.django_db(transaction=True)
def test_feed_sync_threads_public_posts_by_subject(posts_tables: None) -> None:
...
@pytest.mark.django_db(transaction=True)
def test_feed_sync_attributes_reactions_through_the_reaction_owner(posts_tables: None) -> None:
...
@pytest.mark.django_db(transaction=True)
def test_post_metrics_read_derives_from_the_message(posts_tables: None) -> None:
    """A user who can read the post can read its rolled-up engagement counts.

    ``posts/post_metrics`` read derives from ``messaging/message->read`` (like the
