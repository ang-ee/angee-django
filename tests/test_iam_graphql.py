...
from tests.conftest import Client, Credential, ExternalAccount, Vendor
...
def test_available_connections_returns_only_enabled_clients_without_secret_fields(
    iam_connection_tables: None,
) -> None:
    """The public picker is system-scoped but only exposes safe enabled rows."""

    _vendor_and_client("enabled", is_oidc=True, is_enabled=True, client_secret="secret")
    _vendor_and_client("disabled", is_oidc=True, is_enabled=False, client_secret="secret")
...
def test_login_start_rejects_non_oidc_or_disabled_client(
    iam_connection_tables: None,
) -> None:
    """OIDC start fails closed when the selected client cannot run login."""

    _vendor, non_oidc = _vendor_and_client("oauth", is_oidc=False, is_enabled=True)
    _vendor, disabled = _vendor_and_client("off", is_oidc=True, is_enabled=False)
    public_schema = _schema("public")
    query = """
        mutation LoginStart($clientSqid: String!) {
          loginStart(
            clientSqid: $clientSqid,
    ...
    for client in (non_oidc, disabled):
        result = _execute(public_schema, query, {"clientSqid": client.sqid})
...
def test_login_complete_provisions_and_logs_in(
    ...
    _vendor, client = _vendor_and_client("oidc", is_oidc=True, is_enabled=True)
    ...
    def complete_login(
        ...
        *,
        ...
        assert selected_client.pk == client.pk
        assert code == "code"
...
def test_vendor_and_client_crud_are_admin_only(
    iam_connection_tables: None,
) -> None:
    ...
    client = _data(
        _execute(
            console_schema,
            """
            mutation CreateClient($vendor: ID!) {
              createClient(data: {
                vendor: $vendor,
    ...
    assert client["displayName"] == "Console prod"
    assert client["isOidc"] is True


def test_client_secret_never_appears_in_graphql_projection(
    iam_connection_tables: None,
) -> None:
...
def test_my_connected_accounts_are_scoped_to_session_user(
    ...
    vendor, client = _vendor_and_client("scope", is_oidc=True, is_enabled=True)
...
def test_unlink_account_only_removes_callers_credential(
    ...
    vendor, client = _vendor_and_client("unlink", is_oidc=True, is_enabled=True)
...
def _vendor_and_client(
    slug: str,
    **client_overrides: Any,
) -> tuple[Vendor, Client]:
    """Create one vendor and client under system context."""
