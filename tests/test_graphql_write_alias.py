...
@pytest.fixture
def action_writer(transactional_db: None) -> Iterator[str]:
    """Expose a non-default native connection, as in the transition routing tests."""

    del transactional_db
    alias = "graphql_action_writer"
...
def test_authorized_target_passes_selected_alias_to_permission_check(
    ...
    monkeypatch.setattr(Group, "has_access", has_access, raising=False)
    routing = TransitionRouter("unselected_writer")
    monkeypatch.setattr(router, "routers", [routing])
...
def test_hasura_related_preflight_reuses_parent_writer(
    ...
    monkeypatch.setattr(hasura_module.mutation_resolvers, "update", update)
