...
class ParsedThread:
    ...
    prefixes. ``modality``/``visibility`` land on a newly created thread only;
    an established thread keeps its own. ``title`` also fills an established
    thread that has none and never renames one. ``visibility`` is the
