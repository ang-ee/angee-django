...
class ChannelManager(AngeeManager):
    """Owns deleting a connected channel — the purge that removes everything it ingested.

    Deleting the channel row alone would orphan its threads and messages (they FK the
    shared ``integrate.Integration`` parent with ``SET_NULL``). This manager tears down
    that ingested subtree first, then deletes the channel + its ``Integration`` parent row,
    and forecasts the *same* scope by count for the delete-confirmation preview — one owner
    for both the destructive path and its forecast, so the preview cannot drift from the
    delete. ``ThreadManager.teardown_for_channel`` stays the low-level bulk delete; both the
    authored ``delete_channel`` mutation and the generic ``delete_channels_by_pk`` backend
    are thin dispatchers to :meth:`purge`. Bound as ``Channel.deletion`` — a Channel's
    default ``objects`` manager is the inherited ``IntegrationManager``, so these
    channel-specific verbs live on their own additively-named manager.
    """
