...
def _require_retained_object(value: Any, *, label: str) -> None:
    ...
    if value.pk is None or value._state.adding:
        raise ValueError(f"workflows.testing requires a retained object; {label} is not persisted.")
...
def start_run(workflow: Any, *, subject: Any = None, actor: Any = None) -> Any:
    ...
    return engine.start(workflow, subject=subject, actor=actor if actor is not None else current_actor())
...
def advance_once(run: Any, *, now: datetime | None = None) -> list[Any]:
    ...
    with system_context(reason="workflows.testing read started"):
        return list(
            step_run_model.objects.filter(run=run, status=StepRunStatus.STARTED).order_by("pk")
        )
...
def execute_started(
    ...
    *,
    ...
    for row in rows:
        ...
        dispatch_model.objects.deliver(
            dispatch.pk,
...
def run_to_terminal(
    ...
    *,
    ...
    raise AssertionError(f"Workflow run tree did not settle after {max_cycles} cycles: {_run_states(run)}")
