...
export function WebsiteAppearanceTool(): ReactElement {
  ...
  async function useBrand(): Promise<void> {
    ...
    const definition = brandThemes[0];
  ...
  return <Card><CardContent className="grid gap-4">
    ...
    {result ? <div className="grid gap-3"><div><strong>{result.siteName || result.title || result.finalUrl}</strong><div className="text-xs text-fg-muted">{result.finalUrl}</div></div><div className="flex flex-wrap gap-2">{result.colors.map((color) => <span key={color} className="inline-flex items-center gap-1 text-xs"><span className="size-5 rounded-full border border-border" style={{ backgroundColor: color }} />{color}</span>)}</div>{result.neutralTint ? <div className="text-xs text-fg-muted">Neutral tint: {result.neutralTint}</div> : null}{result.fonts.length ? <div className="text-xs text-fg-muted">Fonts: {result.fonts.join(", ")}</div> : null}{result.warnings.map((warning) => <Alert key={warning} tone="warning" title={warning} />)}<Button disabled={!result.colors.length} onClick={() => void useBrand()}>Customize extracted theme</Button></div> : null}
  </CardContent></Card>;
}
