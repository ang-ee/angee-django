"""Rewrite money manager tuples to reference a plain fixed role subject."""
...
_APP_LABEL = "money"
_APP_MODELS = ("currency", "currencyrate")
_RESOURCE_TYPES = ("money/currency", "money/rate")
_ROLE_TYPE = "money/role"
_ROLE_ID = "money_admin"
...
def applies(project_state: ProjectState) -> bool:
    """Apply only when the complete money and REBAC storage state exists."""

    present = [project_state.models.get((_APP_LABEL, name)) for name in _APP_MODELS]
    if not any(present):
        return False
    if not all(present):
        raise ImproperlyConfigured("angee.money:role_subject_relations found partial money state")
    ...
    if relationship is None or registry is None:
        raise ImproperlyConfigured("angee.money:role_subject_relations requires both REBAC stores")
    if not _RELATIONSHIP_FIELDS.issubset(relationship.fields) or not _REGISTRY_FIELDS.issubset(
        registry.fields
    ):
        raise ImproperlyConfigured("angee.money:role_subject_relations found unexpected REBAC state")
    return True
...
def rewrite_manager_subjects(apps: Any, schema_editor: Any) -> None:
    ...
    for model, registry in stores:
        ...
        if registry:
            ...
            identity.update(
                resource_type__in=_RESOURCE_TYPES,
                subject_type=_ROLE_TYPE,
                subject_id=_ROLE_ID,
            )
        for row in rows.filter(**identity).order_by("pk"):
            ...
            if destination is not None and _condition(destination) != _condition(row):
                raise ImproperlyConfigured("angee.money:role_subject_relations found conflicting manager tuples")
...
class Migration(migrations.Migration):
    """Adopt plain fixed-role subjects for money manager relationships."""

    atomic = True
    dependencies = [("money", "__latest__"), ("rebac", "__latest__")]
