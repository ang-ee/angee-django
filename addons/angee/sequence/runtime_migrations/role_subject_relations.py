"""Rewrite sequence manager tuples to reference a plain fixed role subject."""
...
_APP_LABEL = "sequence"
_APP_MODELS = ("sequence",)
_RESOURCE_TYPES = ("sequence/sequence",)
_ROLE_TYPE = "sequence/role"
_ROLE_ID = "sequence_admin"
...
def applies(project_state: ProjectState) -> bool:
    """Apply only when the complete sequence and REBAC storage state exists."""

    present = [project_state.models.get((_APP_LABEL, name)) for name in _APP_MODELS]
    if not any(present):
        return False
    if not all(present):
        raise ImproperlyConfigured("angee.sequence:role_subject_relations found partial sequence state")
    ...
    if relationship is None or registry is None:
        raise ImproperlyConfigured("angee.sequence:role_subject_relations requires both REBAC stores")
    if not _RELATIONSHIP_FIELDS.issubset(relationship.fields) or not _REGISTRY_FIELDS.issubset(
        registry.fields
    ):
        raise ImproperlyConfigured("angee.sequence:role_subject_relations found unexpected REBAC state")
    return True
    ...
    for model, registry in stores:
        ...
        for row in rows.filter(**identity).order_by("pk"):
            ...
            if destination is not None and _condition(destination) != _condition(row):
                raise ImproperlyConfigured("angee.sequence:role_subject_relations found conflicting manager tuples")
...
class Migration(migrations.Migration):
    """Adopt plain fixed-role subjects for sequence manager relationships."""

    atomic = True
    dependencies = [("sequence", "__latest__"), ("rebac", "__latest__")]
