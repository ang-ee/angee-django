...
| neonize | WhatsApp Web multi-device protocol — whatsmeow (Go) via a bundled platform library: QR-code device pairing, the encrypted session store (SQLite), connect/message/receipt event callbacks, media download + decrypt | `messaging_integrate_whatsapp` runs one live session per channel inside the dedicated `whatsapp` Celery queue worker; the session store persists under `ANGEE_DATA_DIR/whatsapp/<channel>/` and events map through the addon parser onto `Message.objects.ingest` |
| qrcode[pil] | QR code generation — segment/error-correction encoding, PNG rendering via Pillow | `messaging_integrate_whatsapp` renders the pairing QR to a PNG data-URI carried in the channel's `sync_progress`; the `[pil]` extra declares the Pillow renderer directly rather than leaning on neonize's transitive Pillow |
