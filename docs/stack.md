...
| jsonschema >= 4.26 + referencing >= 0.37 | Native Draft 2020-12 validation and reference-registry traversal; core wheel dependencies | `angee.base.jsonschema.LocalSchemaReferences` owns root-local scope for `ImplBase` config projections and workflow structural proofs; `workflows_extraction` validates declared and merged evidence; native `workflows` validates frozen tagged Decision actions, branches, and typed context through the referencing registry |
