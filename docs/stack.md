...
| jsonschema >= 4.26 + referencing >= 0.37 | Native Draft 2020-12 validation and reference-registry traversal | `workflows_extraction` validates declared and merged evidence; native `workflows` validates frozen tagged Decision actions, branches, and typed context through the referencing registry |
| defusedxml | Bounded, hardened XML parsing | `workflows_extraction` detects structured document carriers without enabling external entities, DTD expansion, or network retrieval |
...
| pydantic-ai-slim >= 2.13 | Native model/message/usage protocols, structured and multimodal one-shot requests, agent loops and MCP toolsets | `agents.InferenceModel.infer` owns the thin direct-request envelope and normalized usage while preserving native types; `agents.backends` owns provider retry classification; `agents.deployments` owns role-based approval over `InferenceModel.deployment_identity`; vendor addons own SDK and transport policy. Ollama is loopback-only with environment proxies disabled, maps provider `keep_alive` and `generation_limit` through its OpenAI-compatible SDK request, and keeps `num_ctx` in the deployed Modelfile. `agents_runtime_pydantic` adds only the MCP extra and bounded sessions; workflows own durability, journals, budget debits and approvals |
