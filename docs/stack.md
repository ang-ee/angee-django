...
| Django >= 6.0, < 6.1 | ORM, migrations, admin, auth contract, app registry | Abstract bases and build-time composition into runtime apps |
...
| django-countries >= 9, < 10 | Configurable ISO 3166-1 country choices, overrides, localized names, and translated display names | `angee.parties.fields.CountryCodeField` preserves configured codes and names, stores alpha-2 as a plain string, and exposes the upstream choices through resource metadata |
| pycountry >= 26, < 27 | Complete ISO country code and official/common-name lookup | `angee.parties.fields.CountryCodeField` uses the upstream catalogue only when the configured django-countries catalogue has no match |
...
| jsonschema >= 4.26 + referencing >= 0.37 | Native Draft 2020-12 validation and reference-registry traversal; core wheel dependencies | `ImplBase` projects referenced implementation config fields; workflow structural proofs resolve local references natively; `workflows_extraction` validates declared and merged evidence; native `workflows` validates frozen tagged Decision actions, branches, and typed context through the referencing registry |
...
| Celery + Redis | Task transport, worker execution, retries, queue routing, and periodic dispatch | hosted by the `angee.jobs` framework app; the host/stack supplies broker topology; task bodies acquire Angee locks and delegate state changes to model/manager owners. `angee.workflows_integrate` composes `angee.workflows`, `angee.integrate` and `angee.storage` for bounded record pages, archive imports, native workflow retries and durable subject settlement |
...
| django-simple-history | Shadow history tables and revert | [`HistoryMixin`](../angee/base/mixins.py) for abstract source/donor tracking |
...
| httpx | HTTP client/transport for all integrate outbound calls | `integrate.http.PinnedTransport` — an SSRF-pinned httpx transport that resolves once and dials a validated IP (judgement owned by `integrate.net.is_unsafe_address`) with system-store TLS. Composed by `HttpClient` (the integration backends, including bounded method-preserving same-origin redirects) and by the OAuth client (handed to authlib's `OAuth2Client`); the honest `Angee-Integrate/1.0` UA and an injected transport test seam ride on it. `agents` composes native timeout values and transport exception types for inference validation and retry classification |
...
| IMAPClient | IMAP4rev1 protocol client — TLS/STARTTLS, LOGIN/XOAUTH2 auth, modified-UTF-7 folder names, SPECIAL-USE flags, typed UID SEARCH/FETCH responses | `messaging_integrate_imap` drives incremental mailbox sync over it with one `SyncStream` cursor per mailbox; MIME parsing stays on the stdlib `email` package |
...
| neonize | WhatsApp Web multi-device protocol — whatsmeow (Go) via a bundled platform library: QR-code device pairing, the encrypted session store (SQLite), connect/message/receipt event callbacks, media download + decrypt | `messaging_integrate_whatsapp` declares process isolation: the dedicated `whatsapp` Celery worker hosts a fresh interpreter per channel through `angee.integrate.session_process`, so native failures cannot abort other accounts or the queue worker. Cross-process task locks are required. Events map through its parser onto `Message.objects.ingest`; its backend key contributes the `whatsapp` segment to `angee.integrate.live.session_store_path` (`ANGEE_DATA_DIR/<backend key>/<sqid>`) |
...
| matrix-nio[e2e] + vodozemac | Matrix client protocol and E2EE — password login, sync, lazy-member filters, authenticated media, Olm/Megolm decryption of encrypted rooms and attachments over the audited Rust vodozemac ratchets (wheels for every supported platform, no C toolchain) | `messaging_integrate_matrix` puppets one user's own Matrix account on the dedicated `matrix` queue and keeps nio's peewee crypto store under `ANGEE_DATA_DIR/matrix/<sqid>`. ISC + Apache-2.0. vodozemac owns the native cryptography; nio ships no cross-signing, so the addon's `recovery.py` ports mautrix's MPL-2.0 SSSS + self-signing round onto pycryptodome (`pycryptodome` and `unpaddedbase64` are declared directly because that module imports them). E2EE is mandatory and imports fail fast when its boundary is incomplete. |
...
| react-hook-form >= 7.81 | Native values, dirty/touched state, validation execution, errors and submission | Angee field/relation/required policy, atomic line diffs and Refine core read/write transport; resetDefaultValues advances submitted defaults while preserving later edits; the IMAP sample preview composes `DialogForm` with native field validation |
