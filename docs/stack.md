...
| django-countries >= 9 | ISO 3166-1 country codes, name resolution, and translated display names | `angee.parties.fields.CountryCodeField` accepts alpha-2, alpha-3, and upstream-recognized names, stores alpha-2 as a plain string, and exposes the upstream choices through resource metadata |
...
| jsonschema | JSON Schema validation | `workflows_ocr` validates each declared extraction schema and the merged evidence result with the Draft 2020-12 validator |
| defusedxml | Bounded, hardened XML parsing | `workflows_ocr` detects structured invoice carriers without enabling external entities, DTD expansion, or network retrieval |
| pypdfium2 + Pillow | Native PDF text extraction, bounded PDF rasterization, and image normalization | `workflows_ocr` retains text layers directly and turns only scanned pages/images into ordered, capped JPEG recognition inputs |
...
| django-zed-rebac >= 0.17.1 | REBAC engine, Zed introspection/rendering/extension, model subject identity, memberships and garbage collection, live ORM backing and lazy SQL permission scoping | Addon schema composition, resource grant policy and IAM product surfaces through public REBAC APIs |
