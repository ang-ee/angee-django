...
| jsonschema[format-nongpl] >= 4.26 + referencing >= 0.37 | Native Draft 2020-12 validation and reference-registry traversal; the core wheel includes format extras for date-time, uri, hostname, and duration | Core validates implementation schemas; workflows and extraction share the format-asserting validator in `workflows.data_contracts`, with Decision reference registries supplied through the native extension point |
