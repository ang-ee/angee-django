...
| defusedxml | Hardened XML parsing | `angee.parties_integrate_carddav` parses WebDAV/CardDAV XML responses; external consumer ARP accounting intake also uses it for XML carriers |
| pypdfium2 + Pillow | Native PDF text extraction, bounded PDF rasterization, and image normalization | `workflows_extraction` retains text layers directly and turns only scanned pages/images into ordered, capped JPEG recognition inputs; external consumer ARP accounting intake also uses bounded PDF attachment access |
