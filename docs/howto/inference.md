...
For structured output, call `model.require_usable(actor, role)` at the entry
of the authorized operation, then `model.infer(messages, output_schema=schema,
...
`output_schema`, and `settings` restricted to `ModelSettings` keys. A declared
step-level `timeout` takes precedence over `request.settings.timeout`. Transport overrides such as
...
and a live model with the modality required by the role. An absent
...
The model's decoding exception retains the native response for extraction
telemetry. The helper debits usage exactly once, including decoding failures.
